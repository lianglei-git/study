...
vec3 getPosition( float u_time ){ return vec3(     2.5*sin(8.0*u_time), 0.0,      1.0*cos(8.0*u_time) ); }
vec3 getVelocity( float u_time ){ return vec3( 8.0*2.5*cos(8.0*u_time), 0.0, -8.0*1.0*sin(8.0*u_time) ); }
...
void main()
{
  ...
  vec3  rd = normalize( vec3(p.xy,-2.0) );
	...
	vec3  ce = getPosition( u_time );
	vec3  ve = getVelocity( u_time );
  ...
  #define NUMSAMPLES 32
	...
	for( int i=0; i<NUMSAMPLES; i++ )
	{
      ...
      vec3  ce = getPosition( u_time + fi/24.0 );
      vec3 nor = vec3(0.0);
