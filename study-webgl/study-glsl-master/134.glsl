...
{
    if (FragCoord.y/iu_resolution.y>.42)
...
void mainImage(out vec4 fragColor, in vec2 p) {
    ...
    //light
    float theta = (iMouse.x / iu_resolution.x *2. - 1.)*PI;
    float phi = (iMouse.y / iu_resolution.y - .5)*PI;
	...
	if (abs(q.y-.42)*iu_resolution.y<.5)  fragColor = vec4(.75);
    ...
    // display: light
    // float d = length(vec2(lightDir)-p)*iu_resolution.x;
