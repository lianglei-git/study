...
#define SLICE (u_mouse.x * u_resolution.x < gl_FragCoord.x);
...
vec3 position_origin(vec3 p)
{
	...
	if(u_mouse.y < .25)
	{
		p.zy *= rmat(u_mouse.y*TAU*4.-TAU*.5);
		p.xz *= rmat(u_mouse.x*TAU+TAU/2.);
	}
	else
	{
		p.xz *= rmat(u_time * .0625);
		p.yz *= rmat(u_time * .0625);
	}
	return p;
}
...
float map(vec3 position)
{
  ...
  //// multiple shapes on the faces
	...
	float t 	= u_mouse.y < .25 ? u_mouse.x : u_time / 2.;
...
void main( void )
{
	vec2 aspect = u_resolution.xy / u_resolution.yy;
	vec2 uv = gl_FragCoord.xy / u_resolution.xy;
	uv = (uv - .5) * aspect;

	vec2 m = (u_mouse-.5) * aspect;
	...
	//raytrace
	float minimum_range	= 8./max(u_resolution.x, u_resolution.y);
	float max_range = 32.;
