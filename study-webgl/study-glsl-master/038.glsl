...
float distanceFunc(vec3 p){
  // vec3 q = rotate(p, radians(u_time * 20.0), vec3(1.0, 1.0, 1.0));
  // return smoothMin(sdSphere(trans(vec3(p.x, p.y + u_time, p.z))), sdFloor(trans(p)), 5.0);
  return smoothMin(sdSphere(trans(vec3(p.x, p.y - 0.95 + u_time, p.z))), sdFloor(trans(p)), 5.0);
}
