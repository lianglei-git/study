...
uniform sampler2D u_texture_cat1;
uniform sampler2D u_texture_cat2;
...
void main()
{
  ...
  vec2 tUv = gl_FragCoord.xy / u_resolution;
	...
	vec4 disp = texture2D(u_texture_disp, tUv);
	...
	vec4 _texture1 = texture2D(u_texture_cat1, pos1);
	vec4 _texture2 = texture2D(u_texture_cat2, pos2);
