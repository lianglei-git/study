...
void mainImage(out vec4 fragColor, in vec2 fragCoord){
  ...
  vec2 uv = (fragCoord.xy * 2. - iu_resolution.xy) / min(iu_resolution.x, iu_resolution.y);
  // vec2 tUv = fragCoord.xy / iu_resolution;
  ...
  // fragColor = (vec4(dest, 1.) + (texture2D(u_backbuffer, tUv) * .9)) * 1.;
