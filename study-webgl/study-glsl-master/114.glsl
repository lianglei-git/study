...
{
  float n = snoise(vec4(normalize(p), u_time * .5));
  vec3 r = rotate(p, radians(u_time), vec3(1.));
...
void main()
{
  ...
  vec3 lightDir = normalize(vec3(sin(u_time * 10.), cos(u_time * 10.), 1.));
  ...
  vec3 cPos = vec3(.0, .0, cos(u_time * 2.) * .2 + 1.);
  vec3 rPos = cPos;
