...
vec2 distanceFunc(vec3 p){
  ...
  vec3 r = rotate(p, radians(u_time * 20.), vec3(1.));
...
void main(){
  ...
  vec3 lightDir = normalize(vec3(sin(u_time * 20.), cos(u_time * 20.), 1.));
  ...
  float r1 = random(vec2(u_time));
  vec3 fCol = vec3(.7);
  vec3 nCol1 = vec3(normal.z * 1., normal.z * 10., normal.z * (4. - sin(u_time * .5)));
  vec3 nCol2 = vec3(normal.z * 2., normal.z * 1., normal.z * (2. - sin(u_time * .5))) * (cos(u_time * .7) + 1.);
  ...
  gl_FragColor = (vec4(dest, 1.) + (texture2D(u_backbuffer, tUv) * .9)) * 1.;
