...
float distanceFunc(vec3 p){
  ...
  vec3 r = rotate(p, radians(u_time * 50.0), vec3(1.0, 1.0, 1.0));

  vec3 t = twist(p, sin(u_time * 2.0) * 10.0);

  float s1 = sdSphere(p);

  float d1 =
    (sin(2.75 * r.x * 1.0)) * ((sin(u_time * 2.0) * 1.0)) *
    (sin(2.75 * r.y * 1.0)) * ((sin(u_time * 2.0) * 1.0)) *
    (sin(2.75 * r.z * 1.0)) * ((sin(u_time * 2.0) * 1.0));
...
void main()
{
  ...
  if(abs(distance) < 0.001){
    ...
    for(float i = 1.0; i < 3.0; i++){
      p.x += 0.2 / i * sin(i * 1.0 * p.y + u_time * 1.0 + cos((u_time / (100.0 * i)) * i));
      p.y += 0.4 / i * cos(i * 1.0 * p.x + u_time * 2.0 + sin((u_time / (200.0 * i)) * i));
    }
