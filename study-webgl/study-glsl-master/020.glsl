...
int countNeighbors(vec2 p){
  ...
  for (int x=-KERNEL_R; x<=KERNEL_R; ++x){
    ...
    if ( texture2D(u_backbuffer, uv+spoint).a > 0.0 )
      ++count;
  }
...
float gameStep(){
  isLive = texture2D(u_backbuffer, uv).a > 0.0;
...
vec3 ghosting(){
  #define DECAY 0.90
  return DECAY * texture2D(u_backbuffer, uv).rgb;
}

void main()
{
  aspect = u_resolution.x/u_resolution.y;
  uvUnit = 1.0 / u_resolution.xy;

  uv = ( gl_FragCoord.xy / u_resolution.xy );
  pos = (uv - 0.5);
  pos.x *= aspect;

  pmouse = u_mouse - vec2(0.5);
  pmouse.x *= aspect;
