...
float distanceFunc(vec3 p){
  ...
  vec3 rotate1 = rotate(p, radians(u_time * 50.), vec3(1., 1., 1.));
  vec3 twist1 = twist(rotate1, sin(u_time * 2.) * 1.);

  float s1 = sdSphere(vec3(twist1.x + .5 + sin(u_time), twist1.y + .5 + cos(u_time), twist1.z + 1. + sin(u_time)), 1.) * .8;
  float s2 = sdSphere(vec3(twist1.x - .5 - sin(u_time), twist1.y - .5 - cos(u_time), twist1.z - 1. - cos(u_time)), 1.) * .8;
...
void main()
{
  ...
  if(abs(d) < .001){
    ...
    vec3 color = vec3(.1) + vec3(uv + cos(u_time) * .2, 1.);
