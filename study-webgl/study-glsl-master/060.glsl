...
void main()
{
  ...
  mat2 rotation = rotate2d(u_time / 100.);

  float offsetValX = fbm(uv + rotation * vec2(0.0, u_time / speed), seed);
  float offsetValY = fbm(uv + rotation * vec2(10.0, u_time / speed), seed);
  ...
  color += vec3(noiseVal);
  color /= 2.0;
  float gradient = (uv.y + u_time / speed) / 2.5;
  color -= gradient;
